...
public class c4_LifecycleHooks extends LifecycleHooksBase {
    ...
    public void no_subscription_no_gains() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doOnSubscribe(i -> hooksTriggered.add("subscribe")); // when Flux `temperatureFlux` is subscribed too. 이므로 doOnSubscribe 이벤트에서 처리한다.
    ...
    public void be_there_early() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doFirst(() -> hooksTriggered.add("before subscribe")); // doOnSubscribe 보다 빠른 이벤트는 doFirst 이다.
    ...
    public void terminator() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doOnTerminate(hooksTriggeredCounter::incrementAndGet); // take(0) 이면 cancel 되는데 이를 제외하곤 모두 terminate 이벤트가 발생하여 hooksTriggeredCounter 가 증가된다.
...
//                .log()
        ...
        /**
         * doOnEach(), log() 모두 스트림의 신호를 처리하거나 기록하는 데 유용하지만,
         * doOnEach는 보다 세부적인 제어를 제공하고, log는 간편한 로깅 기능을 제공한다.
         */
